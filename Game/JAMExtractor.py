...
def extract(path, verbose):
    ...
    def listFolders(offset, number, folderPath):
        ...
        for _ in range(number):
            ...
            if verbose:
                print("READING: {0}  OFFSET {1}".format(name, position))
    ...
    if os.path.exists(outFolder):
        ...
        i = 1
        while os.path.exists("{0}_{1}_bak".format(outFolder, i)):
            i += 1
        renameFolder = "{0}_{1}_bak".format(outFolder, i)
        print("NOTE: Existing extraction found.\nMOVING: {0} -> {1}".format(
              outFolder, renameFolder))
...
def build(path, verbose):
    ...
    for currentdir, dirlist, filelist in os.walk(path):
        ...
        for filename in filelist:
            ...
            fileList.append(a)
            if verbose:
                print("READING: {0}".format(a[0][len(path):]))
    ...
    outFile = "{0}.JAM".format(path)

    # Move the old file out of the way if it exists.
    if os.path.exists(outFile):
        # Move to the first name not taken.
        i = 1
        while os.path.exists("{0}.{1}.bak".format(outFile, i)):
            i += 1
        renameFile = "{0}.{1}.bak".format(outFile, i)
